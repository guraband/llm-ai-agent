...
def business_analyst(state: State):
    ...
    business_analyst_system_prompt = PromptTemplate.from_template(
        """
        너는 책을 쓰는 AI 팀의 비즈니스 분석가로서,
        AI팀의 진행상황과 '사용자 요구 사항'을 토대로,
        현 시점에서 '지난 요구 사항(previous_user_request)'과 최근 사용자의 발업을 바탕으로 요구사항이 무엇인지 판단한다.
    ...
    ba_message = f"[Business Analyst] {user_request}"
    print(ba_message)
    messages.append(AIMessage(ba_message))
...
def supervisor(state: State):  # supervisor 에이전트 추가
    ...
    # 체인 연결
    supervisor_chain = supervisor_system_prompt | llm. with_structured_output(
        Task)
