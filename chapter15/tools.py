...
def web_search(query: str):
    ...
    if not api_key:
        raise ValueError("TAVILY_API_KEY가 환경변수에 설정되지 않았습니다.")
    ...
    resources_json_path = f'{
        current_path}/data/resources_{datetime.now().strftime('%Y_%m%d_%H%M%S')}.json'
...
def split_documents(documents, chunk_size=1000, chunk_overlap=100):
    ...
    print(f"{len(documents)}개의 문서를 {chunk_size}자 크기로 중첩 {
          chunk_overlap}자로 분할합니다.\n")
...
def documents_to_chroma(documents, chunk_size=1000, chunk_overlap=100):
    ...
    splits = split_documents(
        new_documents, chunk_size=chunk_size, chunk_overlap=chunk_overlap)
